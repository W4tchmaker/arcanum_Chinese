...
export default class RangedMod extends Mod {
	...
	toString() {
		...
		if (+this.step) {
			str += ` 每${precise(this.estimateStep())}件${this.roundingSym ? ` (向${this.roundingSym === "+" ? "上" : "下"}取整)` : ""}`;
		}
		...
		if (+this.min) {
			...
			str += `${this.minOp ? "激活" : "开始"}${this.minOp === OPS.GT.value ? "过" : "于"}${precise(this.min)} (为${this.strBonus(this.start)})`;
		}
		if (+this.max) {
			str += `上限${precise(this.max)} (为${this.adjustedbonus(this.max)})`;
		}
