...
export default {
	...
	onLoot(loot) {
		...
		this.log.log("战利品", text, LOG_LOOT);
	},
	 ...
	 */
	onNewTitle(t, len) {
		this.log.log("已取得头衔 " + uppercase(t), null, LOG_UNLOCK);

		this.dispatch(EVT_STAT, "titles", len);
	},

	actImproved(it) {
		this.log.log(it.name.toTitleCase() + "已提升", null, LOG_UNLOCK);
	},

	onLevel(player, lvl) {
		this.log.log(player.name + "已升级！", null, LOG_EVENT);
	...
	npcSlain(enemy, attacker) {
		this.log.log(
			enemy.name.toTitleCase() + "死亡",
			attacker && attacker?.name?.toTitleCase() ? "因" + attacker.name.toTitleCase() : "",
			LOG_COMBAT,
		);
	},

	onOvercrowded(name, team) {
		this.log.log("人数过多", name.toTitleCase() + " 无法加入 " + team + " 因为人数过多", LOG_COMBAT);
	},
	/**
