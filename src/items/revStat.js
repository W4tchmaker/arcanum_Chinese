...
export default class RevStat extends Resource {
	...
	free() {
		return this.max - this.value - this.delta;
	}

	empty() {
		return this.value + this.delta >= this.max.value;
	}
	 ...
	 */
	canPay(amt) {
		return amt >= 0 ? (this.max ? this.value + amt <= this.max.value : true) : this.value >= -amt;
	}
	remove(amt) {
		this.value.base += amt;
	}
	 ...
	 */
	filled(rate = 0) {
		return this.value + this.delta <= 0 || (this.rate && this.rate + rate >= 0);
	}
