...
import { formatNumber,getMappedGroupName } from "@/util/format";
...
export default {
	...
	computed: {
		displayedGroupTitle() {
            const mappedName = getMappedGroupName(this.school);
            
            if (mappedName === this.school && typeof mappedName.toTitleCase === 'function') {
                return mappedName.toTitleCase();
            }
