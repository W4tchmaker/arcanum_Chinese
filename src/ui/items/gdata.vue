...
export default {
	...
	computed: {
		name() {
			return this.item.sname || this.item.name || this.item.name.toTitleCase();
		},
		damage() {
...
<template>
	<div class="item-info">
		...
		<div class="tight note-text" v-if="item.tags || item.hands">
			<span v-if="item.hands > 1">双手 </span>{{ tags }}
		</div>
		<span
			class="flex-right"
			v-if="
		...
		<span
			...
			">
			效率： {{ precise(item.rate.value * 100) }}%
		</span>
		<div>
			<span class="separate">
			...
			<span v-if="item.at && nextAt > 0" class="note-text">
				升级进度： {{ Math.round((100 * item.value) / nextAt) + "%" }}
			</span>
			<span v-else-if="item.every && nextEvery >= 0" class="note-text">
				升级进度： {{ Math.round(100 * nextEvery) + "%" }}
			</span>

			<div v-if="item.cd || cdRemain > 0" class="note-text">
				冷却： {{ cdRemain > 0 ? cdRemain.toFixed(2) + "Left" : item.cd + "s" }}
			</div>
			<div v-if="cdtags" class="note-text">Shares cooldown with: {{ cdtags }}</div>
			<div v-if="item.exclude" class="note-text">冲突项： {{ this.exclude(item.exclude) }}</div>
		...
		<span v-if="item.length > 0 && item.type === 'task'">
			剩余时长: {{ formatTime(((item.length - item.exp) * 1000) / item.rate) }}
		</span>
		...
		<info v-if="item.buy && !item.owned" :info="item.buy" :checkType="CheckTypes.COST" title="购买消耗" />
		<info v-if="item.cost" :info="item.cost" :checkType="CheckTypes.COST" title="消耗" />
		<info
			v-if="
			...
			title="Sell" />
		<info v-if="item.run" :info="item.run" :checkType="CheckTypes.COST" title="运行时消耗" rate="true" />
		...
		<div v-if="item.disable">
			<div class="info-sect">失效：</div>
			<info :info="item.disable" />
		</div>

		<div v-if="item.enable">
			<div class="info-sect">激活：</div>
